...
const LinkCard: React.FC<LinkCardProps> = ({ link, onDelete, onEdit, userRole, isDraggable }) => {
  ...
  return (
    <div className="group relative rounded-xl border border-[var(--color-border)] bg-[var(--color-card-bg)] p-6 shadow-lg transition-all duration-300 ease-in-out hover:border-[var(--color-accent)]/50 hover:bg-[var(--color-card-bg)] hover:brightness-110 hover:-translate-y-1">
      <div 
      ...
      <div className="relative flex items-center justify-between">
        <div className="flex items-center gap-4 min-w-0">
          ...
          <a
            ...
            className="text-lg font-semibold text-[var(--color-text-primary)] group-hover:text-[var(--color-accent)] transition-colors duration-300 break-words"
            title={link.title}
          >
            {link.title}
          </a>
        ...
        {userRole === 'admin' && (
          <div className="flex-shrink-0 flex items-center gap-1 opacity-70 group-hover:opacity-100 transition-opacity duration-300">
            {isDraggable && (
                <div className="cursor-grab text-[var(--color-text-secondary)] p-2" title="اسحب لإعادة الترتيب">
